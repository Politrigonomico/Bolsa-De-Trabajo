...
<div class="container mx-auto px-4 py-8">
    ...
    <form id="empresaForm" action="{{ route('empresa_nuevo') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="cuit" id="cuit" value="{{ old('cuit') }}" required
                       class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                @error('cuit')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <!-- Rubro (con datalist opcional) -->
            <div>
                <label for="rubro_empresa" class="block text-gray-700 font-medium mb-1">Rubro</label>
                <input list="rubros_empresa" name="rubro_empresa" id="rubro_empresa" value="{{ old('rubro_empresa') }}" required
                       class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <datalist id="rubros_empresa">
                    @foreach($rubros as $item)
                        <option value="{{ $item->rubro }}"></option>
                    @endforeach
                </datalist>
                @error('rubro')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            ...
            <!-- Mail -->
            <div>
                <label for="mail" class="block text-gray-700 font-medium mb-1">Email</label>
                <input type="email" name="mail" id="mail" value="{{ old('mail') }}" required
                       class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                @error('mail')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="mt-6">
            ...
            <textarea name="observacion" id="observacion" rows="4"
                      class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('observacion') }}</textarea>
...
@endsection
>>>>>>> 982a1a3a0157317f3dc9529b2220c8ca1bf75ed0
